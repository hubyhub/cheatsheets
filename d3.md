...
.attr('y', function(d){
	return 300 - d*8;       // um die Säulen von unten des SVG-Elements auszurichten: 300 ist höhe des SVG-ELEMENTS, d*8 ist Höhe der Säule
})				
.attr('width', 20)
.attr('height', function(d){ // 'height' ist in SVG ein attr und kein style
 return d*8; //keine 'px'-Angabe notwendig in SVG
});
```

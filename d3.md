...
Seine Stärken spielt D3.js im Zusammenspiel mit SVG aus. Im Normalfall erstellt man also keine DIVS sondern Rectangles (rect) oder Circles,... in SVG.
...
Um die Änderungen auch wirklich sehen zu können, muß man den Chart re-rendern, indem man
eine Referenz zu all den Kreisen bekommen und alle Properties ändern.
Das macht man wieder mit der `selectAll()`-Methode.
...
**TIPP1:** Übergibt man nicht den Wertn in ms sondern, eine function kann man den delay pro index machen:
```javascript
.delay(function(d,i){        
    return i * 100;
})
...
**TIPP2:** Man kann auch die einzelnen Properties nacheinander animieren. Man braucht nur vor jeder `attr()` die man animieren möchte,
...
svg.selectAll('circle')
    ...
    .style('fill', 'purple');   // man kann auch Farben animieren. Ändert die Farbe gleichzeitig mit dem Radius.
}
```
